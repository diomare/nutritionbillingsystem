...
def configura_professionista():
    ...
    if request.method == "POST":
        ...
        prof.cognome = request.form["cognome"]
        prof.indirizzo = request.form["indirizzo"]
        prof.codice_fiscale = request.form["codice_fiscale"]
        prof.piva = request.form["partita_iva"]
        prof.email = request.form["email"]
        prof.telefono = request.form["telefono"]
    ...
    professionista = carica_professionista()
    return render_template("configura_professionista.html", prof=professionista)
...
def add_invoice():
    ...
    if request.method == 'POST':
        try:
            # Log dei dati inviati dal form
            app.logger.info("Dati inviati dal form: %s", request.form)
            ...
            private_client_id = int(request.form['private_client_id']) if client_type == 'private' and request.form['private_client_id'] else None
            business_client_id = int(request.form['business_client_id']) if client_type == 'business' and request.form['business_client_id'] else None

            # Log per verificare il tipo di cliente e gli ID
            app.logger.info("Tipo di cliente: %s", client_type)
            app.logger.info("ID cliente privato: %s", private_client_id)
            app.logger.info("ID cliente aziendale: %s", business_client_id)

            # Verifica che il client_type sia valido
            if client_type not in ['private', 'business']:
                raise ValueError("Tipo di cliente non valido.")

            # Assicurati che almeno un ID cliente sia presente
            if client_type == 'private' and not private_client_id:
                raise ValueError("ID cliente privato mancante.")
            if client_type == 'business' and not business_client_id:
                app.logger.error("Errore: ID cliente aziendale non valido o mancante.")
                raise ValueError("ID cliente aziendale non valido o mancante.")
...
@app.route("/genera-ts/<int:fattura_id>")
def genera_ts(fattura_id):
    fattura = FATTURE.get(fattura_id)
    if not fattura:
        return jsonify({"errore": "Fattura non trovata"}), 404

    output_path = f"fattura_ts_{fattura_id}.xml"
    genera_xml_ts(fattura, output_path)
...
def generate_report():
    ...
    if report_type == 'invoices':
        ...
        if format_type == 'excel':
            ...
            for client, count, amount in business_clients_pdf:
                business_data.append([
